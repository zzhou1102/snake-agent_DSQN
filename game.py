...
class Snake:
    ...
    def gen_food(self):
        """
        (Snake) -> None
        Generate food in the environment by scanning the screen for available cells.
        """
        # Create a set of all possible positions
        all_positions = set((x, y) for x in range(0, self.w, self.size) for y in range(0, self.h, self.size))

        # Remove positions that are occupied by the snake's body
        available_positions = all_positions - set(self.body)

        # If there are available positions, randomly select one for the food
        if available_positions:
            self.food.append(random.choice(list(available_positions)))

        # If the number of food items exceeds the allowed number, trim the list
